...
//  public Bitmap getBitmapForRecognition() {
//	  return bitmapForRecognition;
//  }
  ...
  private Bitmap getAnnotatedBitmap() {
	...
	int position = 0;
	while(getText().charAt(position) != ']' && position != getText().length())
		position++;
	if(position != getText().length()) {
		String temp = getText().substring(1, position);
		String[] parts = temp.split(",");
		if(parts.length == 4) {
            //+-2 because of strokewidth
			int left   = Integer.parseInt(parts[0]) +1;
			int top    = Integer.parseInt(parts[1]) +1;
			int right  = Integer.parseInt(parts[2]) -1;
			int bottom = Integer.parseInt(parts[3]) -1;
...
//    }
    ...
    public float getConsumedData() {
        return consumedData;
    }

    public void setConsumedData(float consumedData) {
...
//  public void setBitmapForRecognition(Bitmap bitmap) {
...
//  }
    ...
    @Override
    public String toString() {
        return text + " " + recognitionTimeRequired + " " + timestamp;
    }
}
